...
'''
fine_grained_contrastive_loss_v3 :学生Image 对齐教师text, 学生text对齐教师Image
...
class Clip_DALoss_distill_v3(nn.Module):
    ...
    def get_distill_loss_local(self,v_patch_embedding,t_token_embedding,v_patch_embedding_teacher,
        ...
        if self.world_size > 1:
            ...
            #学生Image对齐教师Text
            logits_1,logits_2 = self.get_logits_fine_grained(all_v_patch_embedding,gt_all_t_token_embedding_teacher,all_token_mask)
            #学生Text对齐教师Image
            logits_3,logits_4 = self.get_logits_fine_grained(all_v_patch_embedding_teacher,gt_all_t_token_embedding,all_token_mask)
            
            num_logits = gt_all_t_token_embedding[all_token_mask].shape[0]
            labels = torch.arange(num_logits, device=all_v_patch_embedding.device, dtype=torch.long)
            ...
            # 最终的蒸馏损失
            distill_loss = (
            F.cross_entropy(logits_3, labels) +
            F.cross_entropy(logits_4, labels)
            ) / 2
